...
double distance_squared_pbc(const Box* b, const double a[NDIM], const double c[NDIM])
{
    ...
    for(int d = 0; d < NDIM; ++d){
        ...
        dr -= b->box[d] * round(dr / b->box[d]); //pbc
        r2 += dr * dr;
    }
...
{   // pos is just a point, we need this for particle_transfer,  
    ...
    for(int j = 0; j < b->n; ++j){                
        ...
        if(r2 < 1e-12){
            printf("Error: small distance in particle energy calculation. Exit function.\n");
            exit;
        }
...
double Widom_test(Box*b){
    ...
    for(int i = 0; i <ntests; i++){
        for(int d = 0; d < NDIM; d++){
            r_added[d] = dsfmt_genrand() * (b->box[d]);
        } 

        U_plus = particle_energy_at_position(b, r_added, -1); //-1 so skip self in energy() function doesnt cause us to skip a particle
        
        if((- beta * U_plus) < 700){ //exp overflows at exp(706)
            mu_sum += volume * exp(- beta * U_plus) / (b->n + 1);
            div += 1;
        }
        else{
            printf("- beta * U+ overflow. Value = %lf", - beta * U_plus);
            continue;
            //else mu_sum += volume * exp(- beta * - 100) / (b->n + 1);
        }
    }
    mu_av = mu_sum / div;
    if(mu_av == 0){
...
int main(int argc, char* argv[]){
    ...
    double ratio_volumechange = 100; //1
    double ratio_transfer = 100; //1000
    ...
    int accepted_v = 0; int tried_v = 0;
    int accepted_d = 0;
    int accepted_tr = 0;
    ...
    for(int step=0; step < mc_steps; step++){
        ...
        if(step % output_steps == 0){
            printf("Step %d. Move acceptances: volume %lf (%d tries) \t displ. %lf \t transfer %lf. \n", step, (double)accepted_v/output_steps, tried_v, (double)accepted_d/output_steps, (double)accepted_tr/output_steps);
