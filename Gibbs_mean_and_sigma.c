...
const int    mc_steps        = 2e7;
const int    output_steps    = 1e4;
const int    measures_steps  = 1e5; //200 measure points
const int    mu_measure_steps= 1e4; //10 chempot points per block
const int    eq_check_steps  = 1e6; //cannot be too small because then the blocks are very much correlated and equilibrium will be identified too quickly
const double overall_density = 0.7;
const double delta           = 0.4;     
const double delta_V         = 0.01;
const double r_cut           = 1.25;//2.5; 
const double Temperature     = 0.7;   // roadmap for Temperature: 0.70, 0.80, 0.90, 1.00, 1.10
...
Box gas = { .n = 0, .energy=0, .input_file = "gas.dat",    .label = "gas" , .mu=0, .pressure=0};
Box liq = { .n = 0, .energy=0, .input_file = "liquid.dat", .label = "liq" , .mu=0, .pressure=0};
...
double widom_weight_measurement(const Box* b)
{
    ...
    int counter = 0;
    double sum = 0.0;
    double W = 0.0; //W = <V/N+1 exp (-beta Uplus)> in eq (9) (note the expectation value <>)
    double volume = box_volume(b);
    ...
    for(int i = 0; i < ntest; ++i){
        ...
        if((- beta * info.energy) < 700 && isfinite(exp(-beta * info.energy))) { //avoid overflow
            sum +=  (volume / (b->n + 1)) * exp(-beta * info.energy);
            counter += 1;
        }
    }

    if(counter != 0) W = sum/counter;
...
double standarddev(double l[], int m){
    if(m>1){ 
        double diff_sum = 0.0;
        double mean_of_list = 0.0;
        double sigma_of_list = 0.0;

        for(int j = 0; j < m; j++){ //note m is nr of blocks
            mean_of_list += l[j];
        }
        mean_of_list = mean_of_list / m;

        for (int i = 0; i < m; i++) {
            double diff_gas = l[i] - mean_of_list;
            diff_sum += diff_gas * diff_gas;
            }
        sigma_of_list = sqrt(diff_sum / (m - 1));  

        return sigma_of_list;
        }
    else return 0;
}
...
int main(int argc, char* argv[]){
    ...
    FILE* fp_sigmas = fopen("sigmas.dat", "w");
    if(fp2 == NULL){
        fprintf(stderr, "Error: could not write mu_measurements.dat\n");
        exit(EXIT_FAILURE);
    }
    ...
    fprintf(fp1, "t \t E_tot \t\t rho_gas \t\t rho_liq \t\t n_gas \t\t n_liq \t V_gas \t\t V_liq \t\t P_gas \t\t P_liq \t\t mu_gas \t\t mu_liq  \n"); 
    fprintf(fp2, "t \t W_gas \t\t W_liq \n");
    fprintf(fp_sigmas, "step \t\t average mu_gas \t\t sigma mu_gas \t\t average mu_liq \t\t sigma mu_liq \n");
    ...
    double W_gas = 0.0; 
    double W_liq = 0.0;
    double gas_mean_list[1000];
    double liq_mean_list[1000];
    int W_counter = 0;
    int m = 0; //block index for list of means
    for(int step=0; step < mc_steps; step++){
        ...
        //compute chempot during a block (block is mu_measure_steps - measures_steps / mu_measure_steps long)        
        if(step % mu_measure_steps == 0){
            W_gas += widom_weight_measurement(&gas);
            W_liq += widom_weight_measurement(&liq);
            W_counter += 1;
            //fprintf(fp2, "%d \t %lf \t %lf \n", step, widom_weight_measurement(&gas), widom_weight_measurement(&liq));
        }      

        if(step % measures_steps == 0){
            if(W_counter != 0){
                double W_gas_av = (double)(W_gas/ W_counter); 
                double W_liq_av = (double)W_liq/ W_counter;

                if(W_gas_av != 0 && isfinite(W_gas_av)){
                    gas.mu = - Temperature * log (W_gas_av); //average chempot of this block following eq 9 
                    gas_mean_list[m] = gas.mu; //needed to compute standard dev at the end
                }
                
                if(W_liq_av != 0 && isfinite(W_liq_av)){
                    liq.mu = - Temperature * log ( W_liq_av); 
                    liq_mean_list[m] = liq.mu;
                }       
                m++;     
        
            W_gas = W_liq = W_counter = 0;
            }
            
            double volume_gas = gas.box[0]*gas.box[1]*gas.box[2];
            double volume_liq = liq.box[0]*liq.box[1]*liq.box[2];
            fprintf(fp1, "%d \t %lf \t %lf \t %lf \t %d \t %d \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \n", step, E_tot, (double)(gas.n/volume_gas), (double)(liq.n/volume_liq), gas.n, liq.n, volume_gas, volume_liq, pressure_measurement(&gas), pressure_measurement(&liq), gas.mu, liq.mu); 
        ...
        if(step % output_steps == 0){
            vol_print_count++; 
            if(step > 0){
                printf("Step %lf million. Move acceptance: displacement acceptance = %lf. \t transfer acceptance = %lf. \t", (double)(step/1e6), (double)accepted_displacement/tot_disp, (double)accepted_transfer/tot_transf);
        ...
        if(step % eq_check_steps ==0){
            printf("checking for equilibrium now, step = %d\n", step);
            //compute the mean chempot and std SO FAR
            double sigma_of_gas_list = standarddev(gas_mean_list, m);
            double sigma_of_liq_list = standarddev(liq_mean_list, m);

            //could be optimized by making the standarddev return the mean as well with a structure but it shouldnt take too long to compute again
            double mean_of_gas_list = 0.0;
            double mean_of_liq_list = 0.0;
            for(int j = 0; j < m; j++){ //note m is nr of blocks.
                mean_of_gas_list += gas_mean_list[j];
                mean_of_liq_list += liq_mean_list[j];
            }
            if(m!=0){
                mean_of_gas_list = mean_of_gas_list / m;
                mean_of_liq_list = mean_of_liq_list / m;
            }
            
            fprintf(fp_sigmas, "%d \t %lf \t %lf \t %lf \t %lf \n", step, mean_of_gas_list, sigma_of_gas_list, mean_of_liq_list, sigma_of_liq_list);

            //check if equilibrium is reached: i.e. if abs(mu_gas - mu_liq)  <= sqrt(sigma_gas2 + sigma_liq2)
            double diff_means = fabs(mean_of_gas_list - mean_of_liq_list);
            double combined_sigma = sqrt(sigma_of_gas_list * sigma_of_gas_list +
                                sigma_of_liq_list * sigma_of_liq_list);

            if (diff_means <= combined_sigma) {
                    printf("Yay! Equilibrium was reached between %d and %d steps \n", (step-eq_check_steps), step);
